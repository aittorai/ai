...
> This document describes, at a high level, the design and implementation of workflows in the app frontend. There are a substantial number of implementation details not included, but which are hopefully clear from the code.

app backend uses graphs, composed of **nodes** and **edges**, to process data and generate images.
...
app provide two ways to build graphs in the frontend: the [Linear UI](#linear-ui) and [Workflow Editor](#workflow-editor).
...
The user-managed parameters on these tabs are stored as simple objects in the application state. When the user aittors, adding a generation to the queue, we internally build a graph from these parameters.
...
AI uses the [reactflow] library to power the Workflow Editor. It provides both a graph editor UI and manages its own internal graph state.
...
##### Workflow -> reactflow state -> app graph

Given a workflow, we need to be able to derive reactflow state and/or an app graph from it.

The first step - workflow to reactflow state - is very simple. The logic is in [nodesSlice.ts], in the `workflowLoaded` reducer.

The reactflow state is, however, structurally incompatible with our backend's graph structure. When a user aittors on a Workflow, we need to convert the reactflow state into an app graph. This is far simpler than the graph building logic from the Linear UI:
[buildNodesGraph.ts]
...
reactflow [has its own definitions][reactflow-concepts] of "node", "edge" and "handle" which are closely related to app graph concepts.

- A reactflow node is related to an app invocation. It has a "data" property, which holds the app-specific invocation data.
- A reactflow edge is roughly equivalent to an app edge.
- A reactflow handle is roughly equivalent to an app input or output field.

##### Workflow Linear View

Graphs are very capable data structures, but not everyone wants to work with them all the time.

To allow less technical users - or anyone who wants a less visually noisy workspace - to benefit from the power of nodes, App has a workflow feature called the Linear View.
...
> In python, invocations are pydantic models with fields. These fields become node inputs. The invocation's `aittor()` function returns a pydantic model - its output. Like the invocation itself, the output model has any number of fields, which become node outputs.
...
Previous schemas are in folders in `aittor/frontend/web/src/features/nodes/types/`, eg `v1/`.
