...
learning models used by AI. It consists of a series of
...
* _ModelInstallServiceBase_ A service for installing models to
  ...
  information. It is also responsible for managing the app
...
`aittor/app/services` in the following directories:

* `aittor/app/services/model_records/`
* `aittor/app/services/model_install/`
* `aittor/app/services/downloads/`
* `aittor/app/services/model_load/`

Code related to the FastAPI web API can be found in
`aittor/app/api/routers/model_manager_v2.py`.
...
are defined in `aittor.backend.model_manager.config`. They are also
imported by, and can be reexported from,
`aittor.app.services.model_manager.model_records`:

```
from aittor.app.services.model_records import ModelType, ModelFormat, BaseModelType
```
...
is taken to be relative to the app root directory
...
either `aittor.app.services.model_records` or
`aittor.backend.model_manager.config`.
...
`aittor.app.services.model_records` or
`aittor.backend.model_manager.config`.
...
code, but available in `aittor.backend.model_manager.config` if
...
`ApiDependencies.operator.services.model_manager.store`.
...
```
from aittor.app.services.model_records import ModelRecordServiceSQL, ModelRecordServiceFile
...
case of storing the record information in the main app database,
usually `databases/app.db`.
...
for use in the app web server. Its signature is:
...
2. If `model_config_db` is `auto` (the default), then:
   ...
   * Open up a new connection to `databases/app.db` if `conn`
...
4. If `model_config_db` is None, then retrieve the legacy
   `conf_path` option from `aittor.yaml` and use the Path
...
```
import sqlite3
from aittor.app.services.thread import lock
from aittor.app.services.model_records import ModelRecordServiceBase
...
* Installing a model alreadiy located on the local filesystem, by
  moving it into the app root directory under the
...
* Interface with the app event bus to provide status updates on
...
* Saving tags and other metadata about the model into the app database
  when fetching from a repo that provides that type of information,
  (currently only HuggingFace).
  
### Initializing the installer

A default installer is created at app api startup time and stored
in `ApiDependencies.operator.services.model_install` and can
...
```
from aittor.app.services.config import get_config
from aittor.app.services.model_records import ModelRecordServiceSQL
from aittor.app.services.model_install import ModelInstallService
from aittor.app.services.download import DownloadQueueService
from aittor.app.services.shared.sqlite.sqlite_database import SqliteDatabase
from aittor.backend.util.logging import AppAILogger

config = get_config()

logger = AppAILogger.get_logger(config=config)
...
|------------------|------------------------------|------------------------------|
| `app_config`         | AIAppConfig       | app configuration object |
...
```
from aittor.app.services.model_install import (
 LocalModelSource,
 HFModelSource,
 URLModelSource,
)
...
| `inplace`        | bool                         | False       | If set, the model file(s) will be left in their location; otherwise they will be copied into the app root's `models` directory |
...
imported from `aittor.backend.model_manager` and has the following
values:
...
app may not be able to load and run all variants. At the current
...
broadcast to the app event bus. The events will appear on the bus
...
The install service uses the `aittor.backend.model_manager.probe`
...
Properties that provide access to the installer's `AIAppConfig`,
...
the model inside the app models directory before registering
it.
...
they are inside or outside the app models hierarchy.
...
`APP_ROOT/models/.cache`. If the requested file is found in the
...
exceeds the value specified in app `convert_cache` configuration
variable.

#### installer.start(operator)
...
app can download arbitrary files using a multithreaded background
...
`aittor.app.services.download_manager`. However, most of the
implementation is spread out among several files in
`aittor/backend/model_manager/download/*`
...
occurred). Note that since all app model install operations
...
```
from aittor.app.services.download_manager import DownloadQueueService
...
The modules found under `aittor.backend.model_manager.metadata`
...
model's config, as defined in `aittor.backend.model_manager.config`.
...
from app.app.api.dependencies import ApiDependencies
...
and stored in
`ApiDependencies.operator.services.model_loader`. However, you can
create alternative instances if you wish.

### Creating a ModelLoadService object

The class is defined in
`aittor.app.services.model_load`. It is initialized with
an AIAppConfig object, from which it gets configuration
...
```
from aittor.app.services.config import AIAppConfig
from aittor.app.services.model_load import ModelLoadService, ModelLoaderRegistry
...
`aittor/backend/model_manager/load/model_loaders`. Inherit from
...
found in the global `ApiDependencies.operator.services.model_manager`
...
```
mm = ApiDependencies.operator.services.model_manager
```
