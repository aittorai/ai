...
BOOTSTRAP_VENV_PREFIX = "app-installer-tmp"
...
class Installer:
    """
    Deploys an Aittor installation into a given path
    """
    ...
    def app_venv(self, venv_parent: Path) -> Path:
        """
        Create a virtualenv for the Aittor installation
        """

        venv_dir = venv_parent / ".venv"

        # Prefer to copy python executables
        # so that updates to system python don't break Aittor
        try:
    ...
    def install(
        ...
        """Install the Aittor application into the given runtime path
        ...
        auto_dest = Path(os.environ.get("APP_ROOT", root)).expanduser().resolve()
        ...
        self.instance = AittorInstance(runtime=destination, venv=self.venv, version=version)

        # install dependencies and the Aittor application
...
To start the application, run:
    {destination}/run.{"bat" if sys.platform == "win32" else "sh"}

"""
        print(message)


class AittorInstance:
    """
    Manages an installed instance of Aittor, comprising a virtual environment and a runtime directory.
    ...
    def __init__(self, runtime: Path, venv: Path, version: str = "stable") -> None:
        ...
        set_sys_path(venv)
        os.environ["APP_ROOT"] = str(self.runtime.expanduser().resolve())
    ...
    def install(
        ...
        messages.simple_banner("Installing the Aittor Application :art:")
        ...
        try:
            ...
            print(f"Error: {e}")
            print(
                "Could not install Aittor. Please try downloading the latest version of the installer and install again."
            )
            sys.exit(1)
...
def get_github_releases() -> tuple[list[str], list[str]] | None:
    ...
    url = "https://api.github.com/repos/aittorai/AI/releases"
    releases: list[str] = []
