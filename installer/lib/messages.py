...
def welcome(available_releases: tuple[list[str], list[str]] | None = None) -> None:
    ...
    print(
        Panel(
            title="[bold wheat1]Welcome to the Aittor Installer",
            renderable=text(),
            box=box.DOUBLE,
            expand=True,
            padding=(1, 2),
...
def installing_from_wheel(wheel_filename: str) -> None:
    ...
    def text():
        ...
        yield Text.from_markup(
            "[bold orange3]If you are not sure why you are doing this, you should cancel and install Aittor normally."
        )
...
def choose_version(available_releases: tuple[list[str], list[str]] | None = None) -> str:
    """
    Prompt the user to choose an app version to install
    """
    ...
    console.print(":grey_question: [orange3]Please choose an app version to install.")
...
def confirm_install(dest: Path) -> bool:
    if dest.exists():
        ...
        print(f":file_folder: Aittor will be installed in {dest}")
        default = True
            ...
            Group(
                "\n".join(
                    [
                        ...
                        "",
                        "See [deep_sky_blue1]https://aittor.com/ai/#system[/] to ensure your system meets the minimum requirements.",
                        "",
                        "[red3]🠶[/] [b]Your GPU drivers must be correctly installed before using Aittor![/] [red3]🠴[/]",
                    ]
                ),
                "",
    ...
    console.print(
        Panel(
            Group(
                "\n".join(
                    [
                        "We will now apply a registry fix to enable long paths on Windows. Aittor needs this to function correctly. We are asking your permission to modify the Windows Registry on your behalf.",
                        "",
