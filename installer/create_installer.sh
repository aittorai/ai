...
VERSION=$(
    cd ..
    python3 -c "from aittor.version import __version__ as version; print(version)"
)
VERSION="v${VERSION}"
...
pushd ../aittor/frontend/web >/dev/null
...
echo
echo "Building installer zip files for app ${VERSION}..."
echo
...
for f in templates *.txt *.reg; do
    cp -r ${f} app-Installer/
done
mkdir app-Installer/lib
cp lib/*.py app-Installer/lib

# Install scripts
# Mac/Linux
cp install.sh.in app-Installer/install.sh
chmod a+x app-Installer/install.sh

# Windows
cp install.bat.in app-Installer/install.bat
cp WinLongPathsEnabled.reg app-Installer/

FILENAME=app-installer-$VERSION.zip

# Zip everything up
zip -r ${FILENAME} app-Installer
...
if [[ -z ${CI} ]]; then
    ...
    rm -rf app-Installer tmp ../aittor/frontend/web/dist/
fi
