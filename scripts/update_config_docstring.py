...
def generate_config_docstrings() -> str:
    """Helper function for mkdocs. Generates a docstring for the AIAppConfig class.
    ...
    """
    docstring = '    """Global app configuration.\n\n'
    docstring += "    Typically, you won't need to interact with this class directly. Instead, use the `get_config` function from `aittor.app.services.config` to get a singleton config object.\n\n"
    docstring += "    Attributes:\n"

    field_descriptions: list[str] = []
    type_hints = get_type_hints(AIAppConfig)

    for k, v in AIAppConfig.model_fields.items():
...
def main():
    ...
    with open("aittor/app/services/config/config_default.py", "r") as f:
        lines = f.readlines()

    # Find the class definition line
    class_def_index = next(i for i, line in enumerate(lines) if "class AIAppConfig" in line)
    ...
    with open("aittor/app/services/config/config_default.py", "w") as f:
        f.writelines(lines)
