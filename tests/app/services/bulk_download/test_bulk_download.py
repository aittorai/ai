...
from aittor.app.services.board_records.board_records_common import BoardRecord, BoardRecordNotFoundException
from aittor.app.services.bulk_download.bulk_download_common import BulkDownloadTargetException
from aittor.app.services.bulk_download.bulk_download_default import BulkDownloadService
from aittor.app.services.events.events_common import (
...
from aittor.app.services.images.images_common import ImageDTO
...
def mock_temporary_directory(monkeypatch: Any, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "aittor.app.services.bulk_download.bulk_download_default.TemporaryDirectory", mock_TemporaryDirectory
    )
...
def test_handler_image_names(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    ...
    expected_zip_path, expected_image_path, mock_image_contents = prepare_handler_test(
        tmp_path, monkeypatch, mock_image_dto, mock_operator
    )

    bulk_download_service = BulkDownloadService()
    bulk_download_service.start(mock_operator)
    bulk_download_service.handler([mock_image_dto.image_name], None, None)

    assert_handler_success(
        expected_zip_path, expected_image_path, mock_image_contents, tmp_path, mock_operator.services.events
    )
...
def test_generate_id(monkeypatch: Any):
    ...
    monkeypatch.setattr("aittor.app.services.bulk_download.bulk_download_default.uuid_string", lambda: "test")

    assert bulk_download_service.generate_item_id(None) == "test"


def test_generate_id_with_board_id(monkeypatch: Any, mock_operator: Operator):
    ...
    monkeypatch.setattr(mock_operator.services.board_records, "get", mock_board_get)
...
def test_handler_board_id(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    ...
    monkeypatch.setattr(mock_operator.services.images, "get_many", mock_get_many)
...
def test_handler_board_id_default(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    """Test that the handler creates the zip file correctly when given a board id."""
...
def test_handler_bulk_download_item_id_given(
    tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator
):
...
def prepare_handler_test(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    ...
    monkeypatch.setattr("aittor.app.services.bulk_download.bulk_download_default.uuid_string", mock_uuid_string)
    ...
    monkeypatch.setattr(mock_operator.services.images, "get_dto", mock_get_dto)
    ...
    monkeypatch.setattr(
        mock_operator.services.board_image_records,
    ...
    monkeypatch.setattr(mock_operator.services.images, "get_path", mock_get_path)
...
def test_handler_on_image_not_found(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    ...
    execute_handler_test_on_error(tmp_path, monkeypatch, mock_image_dto, mock_operator, exception)


def test_handler_on_board_not_found(tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator):
    """Test that the handler emits an error event when the image is not found."""
    ...
    monkeypatch.setattr(mock_operator.services.images, "get_dto", mock_get_board_name)
...
def test_handler_on_generic_exception(
    ...
    event_bus: TestEventService = mock_operator.services.events
...
def execute_handler_test_on_error(
    tmp_path: Path, monkeypatch: Any, mock_image_dto: ImageDTO, mock_operator: Operator, error: Exception
):
    bulk_download_service = BulkDownloadService()
    bulk_download_service.start(mock_operator)
    bulk_download_service.handler([mock_image_dto.image_name], None, None)
