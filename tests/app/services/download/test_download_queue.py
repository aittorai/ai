...
from aittor.app.services.config.config_default import URLRegexTokenPair
from aittor.app.services.download import DownloadJob, DownloadJobStatus, DownloadQueueService, MultiFileDownloadJob
...
from aittor.backend.model_manager.metadata import HuggingFaceMetadataFetch, ModelMetadataWithFiles, RemoteModelFile
...
def test_tokens(tmp_path: Path, mm2_session: Session):
    with clear_config():
        ...
        queue.join()
        # this token is defined in the temporary root aittor.yaml
        # see tests/backend/model_manager/data/app_root/aittor.yaml
