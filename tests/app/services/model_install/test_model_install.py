...
from aittor.app.services.model_records import ModelRecordChanges, UnknownModelException
...
def test_install(
    mm2_installer: ModelInstallServiceBase, embedding_file: Path, mm2_app_config: AIAppConfig
) -> None:
    store = mm2_installer.record_store
...
def test_simple_download(mm2_installer: ModelInstallServiceBase, mm2_app_config: AIAppConfig) -> None:
...
@pytest.mark.timeout(timeout=10, method="thread")
def test_huggingface_install(mm2_installer: ModelInstallServiceBase, mm2_app_config: AIAppConfig) -> None:
...
@pytest.mark.timeout(timeout=10, method="thread")
def test_huggingface_repo_id(mm2_installer: ModelInstallServiceBase, mm2_app_config: AIAppConfig) -> None:
...
def test_404_download(mm2_installer: ModelInstallServiceBase, mm2_app_config: AIAppConfig) -> None:
...
def test_other_error_during_install(
    monkeypatch: pytest.MonkeyPatch, mm2_installer: ModelInstallServiceBase, mm2_app_config: AIAppConfig
) -> None:
    def raise_runtime_error(*args, **kwargs):
        raise RuntimeError("Test error")

    monkeypatch.setattr(
        "aittor.app.services.model_install.model_install_default.ModelInstallService._register_or_install",
        raise_runtime_error,
    )
...
@pytest.mark.parametrize(
    ...
    [
        # SDXL, Lora
        {
            "repo_id": "Aittor-test/textual_inversion_tests::learned_embeds-steps-1000.safetensors",
            "name": "test_lora",
            "type": "embedding",
        },
