...
from aittor.app.invocations.baseinvocation import BaseInvocation, BaseInvocationOutput, InvocationContext
from aittor.app.invocations.collections import RangeInvocation
from aittor.app.invocations.math import AddInvocation, MultiplyInvocation
from aittor.app.services.shared.graph import (
    CollectInvocation,
    Graph,
    GraphExecutionState,
    IterateInvocation,
...
# This import must happen before other aittor imports or test in other files(!!) break
...
def aittor_next(g: GraphExecutionState) -> tuple[Optional[BaseInvocation], Optional[BaseInvocationOutput]]:
    n = g.next()
    if n is None:
        return (None, None)

    print(f"invoking {n.id}: {type(n)}")
    o = n.aittor(Mock(InvocationContext))
    g.complete(n.id, o)
