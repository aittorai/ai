...
from aittor.app.services.config.config_default import (
    DefaultAppConfig,
    AIAppConfig,
    get_config,
...
def test_path_resolution_root_not_set(patch_rootdir: None):
    ...
    config = AIAppConfig()
    expected_root = AIAppConfig.find_root()
...
def test_read_config_from_file(tmp_path: Path, patch_rootdir: None):
    ...
    temp_config_file = tmp_path / "temp_aittor.yaml"
...
def test_write_config_to_file(patch_rootdir: None):
    ...
    with TemporaryDirectory() as tmpdir:
        temp_config_path = Path(tmpdir) / "aittor.yaml"
        config = AIAppConfig(host="192.168.1.1", port=8080)
...
def test_update_config_with_object(patch_rootdir: None):
    ...
    config = AIAppConfig()
    new_config = AIAppConfig(host="10.10.10.10", port=6060)
...
def test_set_and_resolve_paths(patch_rootdir: None):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        assert config.db_path == Path(tmpdir).resolve() / "databases" / "aittor.db"
...
def test_get_config_writing(patch_rootdir: None, monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    ...
    config = get_config()
    get_config.cache_clear()
    config_file_path = tmp_path / "aittor.yaml"
...
def test_deny_nodes(patch_rootdir):
    ...
    from aittor.app.services.shared.graph import Graph
