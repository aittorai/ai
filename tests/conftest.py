...
def mock_services() -> InvocationServices:
    configuration = AIAppConfig(use_memory_db=True, node_cache_size=0)
...
@pytest.fixture()
def mock_operator(mock_services: InvocationServices) -> Operator:
    return Operator(services=mock_services)


@pytest.fixture(scope="module")
def app_root_dir(tmp_path_factory) -> Path:
    root_template = Path(__file__).parent.resolve() / "backend/model_manager/data/app_root"
