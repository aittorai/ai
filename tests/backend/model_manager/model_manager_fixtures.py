...
from aittor.app.services.download import DownloadQueueService, DownloadQueueServiceBase
from aittor.app.services.model_install import ModelInstallService, ModelInstallServiceBase
from aittor.app.services.model_load import ModelLoadService, ModelLoadServiceBase
from aittor.app.services.model_manager import ModelManagerService, ModelManagerServiceBase
from aittor.app.services.model_records import ModelRecordServiceBase, ModelRecordServiceSQL
...
from aittor.backend.model_manager.load import ModelCache
...
# Create a temporary directory using the contents of `./data/app_root` as the template
@pytest.fixture
def mm2_root_dir(tmp_path_factory) -> Path:
    root_template = Path(__file__).resolve().parent / "data" / "app_root"
    temp_dir: Path = tmp_path_factory.mktemp("data") / "app_root"
...
@pytest.fixture
def mm2_app_config(mm2_root_dir: Path) -> AIAppConfig:
    app_config = AIAppConfig(models_dir=mm2_root_dir / "models", log_level="info")
...
@pytest.fixture
def mm2_loader(mm2_app_config: AIAppConfig) -> ModelLoadServiceBase:
...
@pytest.fixture
def mm2_record_store(mm2_app_config: AIAppConfig) -> ModelRecordServiceBase:
    logger = AppLogger.get_logger(config=mm2_app_config)
    db = create_mock_sqlite_database(mm2_app_config, logger)
    store = ModelRecordServiceSQL(db, logger)
...
def mm2_session(embedding_file: Path, diffusers_dir: Path) -> Session:
    ...
    sess.mount(
        "https://huggingface.co/api/models/Aittor-test/textual_inversion_tests?blobs=True",
        TestAdapter(
            HFTestLoraMetadata,
    ...
    sess.mount(
        "https://huggingface.co/Aittor-test/textual_inversion_tests/resolve/main/learned_embeds-steps-1000.safetensors",
        TestAdapter(
            data,
