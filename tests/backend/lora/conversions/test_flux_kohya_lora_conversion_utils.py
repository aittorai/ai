...
from aittor.backend.lora.conversions.flux_kohya_lora_conversion_utils import (
    convert_flux_kohya_state_dict_to_aittor_format,
...
def test_convert_flux_kohya_state_dict_to_aittor_format():
    # Construct state_dict from state_dict_keys.
    state_dict = keys_to_mock_state_dict(flux_kohya_state_dict_keys)

    converted_state_dict = convert_flux_kohya_state_dict_to_aittor_format(state_dict)
...
def test_convert_flux_kohya_state_dict_to_aittor_format_error():
    """Test that an error is raised by convert_flux_kohya_state_dict_to_aittor_format() if the input state_dict contains
    unexpected keys.
    """
    state_dict = {
    ...
    with pytest.raises(ValueError):
        convert_flux_kohya_state_dict_to_aittor_format(state_dict)
