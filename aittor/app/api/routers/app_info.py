...
from aittor.app.invocations.upscale import ESRGAN_MODELS
from aittor.app.services.invocation_cache.invocation_cache_common import InvocationCacheStatus
from aittor.backend.image_util.infill_methods.patchmatch import PatchMatch
from aittor.backend.util.logging import logging
...
async def get_log_level() -> LogLevel:
    ...
    return LogLevel(ApiDependencies.operator.services.logger.level)
...
async def set_log_level(
    ...
    ApiDependencies.operator.services.logger.setLevel(level)
    return LogLevel(ApiDependencies.operator.services.logger.level)
...
async def clear_invocation_cache() -> None:
    ...
    ApiDependencies.operator.services.invocation_cache.clear()
...
async def enable_invocation_cache() -> None:
    """Clears the invocation cache"""
    ApiDependencies.operator.services.invocation_cache.enable()
...
async def disable_invocation_cache() -> None:
    """Clears the invocation cache"""
    ApiDependencies.operator.services.invocation_cache.disable()
...
async def get_invocation_cache_status() -> InvocationCacheStatus:
    """Clears the invocation cache"""
    return ApiDependencies.operator.services.invocation_cache.get_status()
