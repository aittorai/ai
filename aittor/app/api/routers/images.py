...
from aittor.app.services.images.images_common import ImageDTO, ImageUrlsDTO
...
async def upload_image(
    ...
    try:
        ...
        ApiDependencies.operator.services.logger.error(traceback.format_exc())
        raise HTTPException(status_code=415, detail="Failed to read image")

    # TODO: retain non-aittor metadata on upload?
    # attempt to parse metadata from image
    metadata_raw = metadata if isinstance(metadata, str) else pil_image.info.get("app_metadata", None)
    if isinstance(metadata_raw, str):
        _metadata = metadata_raw
    else:
        ApiDependencies.operator.services.logger.debug("Failed to parse metadata for uploaded image")
        pass

    # attempt to parse workflow from image
    workflow_raw = pil_image.info.get("app_workflow", None)
    if isinstance(workflow_raw, str):
        _workflow = workflow_raw
    else:
        ApiDependencies.operator.services.logger.debug("Failed to parse workflow for uploaded image")
        pass

    # attempt to extract graph from image
    graph_raw = pil_image.info.get("app_graph", None)
    if isinstance(graph_raw, str):
        _graph = graph_raw
    else:
        ApiDependencies.operator.services.logger.debug("Failed to parse graph for uploaded image")
        pass

    try:
        image_dto = ApiDependencies.operator.services.images.create(
            image=pil_image,
...
async def delete_image(
    ...
    try:
        ApiDependencies.operator.services.images.delete(image_name)
    except Exception:
...
async def clear_intermediates() -> int:
    ...
    try:
        count_deleted = ApiDependencies.operator.services.images.delete_intermediates()
...
async def get_intermediates_count() -> int:
    ...
    try:
        return ApiDependencies.operator.services.images.get_intermediates_count()
    except Exception:
...
async def update_image(
    ...
    try:
        return ApiDependencies.operator.services.images.update(image_name, image_changes)
    except Exception:
...
async def get_image_dto(
    ...
    try:
        return ApiDependencies.operator.services.images.get_dto(image_name)
    except Exception:
...
async def get_image_metadata(
    ...
    try:
        return ApiDependencies.operator.services.images.get_metadata(image_name)
    except Exception:
        raise HTTPException(status_code=404)
...
async def get_image_workflow(
    ...
    try:
        workflow = ApiDependencies.operator.services.images.get_workflow(image_name)
        graph = ApiDependencies.operator.services.images.get_graph(image_name)
...
async def get_image_full(
    ...
    try:
        path = ApiDependencies.operator.services.images.get_path(image_name)
...
async def get_image_thumbnail(
    ...
    try:
        path = ApiDependencies.operator.services.images.get_path(image_name, thumbnail=True)
        with open(path, "rb") as f:
            content = f.read()
...
async def get_image_urls(
    ...
    try:
        image_url = ApiDependencies.operator.services.images.get_url(image_name)
        thumbnail_url = ApiDependencies.operator.services.images.get_url(image_name, thumbnail=True)
        return ImageUrlsDTO(
...
async def list_image_dtos(
    ...
    image_dtos = ApiDependencies.operator.services.images.get_many(
...
async def star_images_in_list(
    ...
    try:
        ...
        for image_name in image_names:
            try:
                ApiDependencies.operator.services.images.update(image_name, changes=ImageRecordChanges(starred=True))
...
async def unstar_images_in_list(
    ...
    try:
        ...
        for image_name in image_names:
            try:
                ApiDependencies.operator.services.images.update(image_name, changes=ImageRecordChanges(starred=False))
                updated_image_names.append(image_name)
            except Exception:
                pass
        return ImagesUpdatedFromListResult(updated_image_names=updated_image_names)
...
async def download_images_from_list(
    ...
    bulk_download_item_id: str = ApiDependencies.operator.services.bulk_download.generate_item_id(board_id)

    background_tasks.add_task(
        ApiDependencies.operator.services.bulk_download.handler,
        image_names,
        board_id,
...
async def get_bulk_download_item(
    ...
    try:
        path = ApiDependencies.operator.services.bulk_download.get_path(bulk_download_item_name)
        ...
        response.headers["Cache-Control"] = f"max-age={IMAGE_MAX_AGE}"
        background_tasks.add_task(ApiDependencies.operator.services.bulk_download.delete, bulk_download_item_name)
        return response
    except Exception:
        raise HTTPException(status_code=404)
