...
from aittor.app.services.model_images.model_images_common import ModelImageFileNotFoundException
from aittor.app.services.model_install.model_install_common import ModelInstallJob
from aittor.app.services.model_records import (
...
from aittor.backend.model_manager.load.model_cache.model_cache_base import CacheStats
from aittor.backend.model_manager.metadata.fetch.huggingface import HuggingFaceMetadataFetch
from aittor.backend.model_manager.metadata.metadata_base import ModelMetadataWithFiles, UnknownMetadataException
from aittor.backend.model_manager.search import ModelSearch
from aittor.backend.model_manager.starter_models import STARTER_MODELS, StarterModel, StarterModelWithoutDependencies
...
def add_cover_image_to_model_config(config: AnyModelConfig, dependencies: Type[ApiDependencies]) -> AnyModelConfig:
    ...
    cover_image = dependencies.operator.services.model_images.get_url(config.key)
...
async def list_model_records(
    ...
    record_store = ApiDependencies.operator.services.model_manager.store
...
async def get_model_records_by_attrs(
    ...
    configs = ApiDependencies.operator.services.model_manager.store.search_by_attr(
...
async def get_model_record(
    ...
    """Get a model record"""
    try:
        config = ApiDependencies.operator.services.model_manager.store.get_model(key)
...
async def scan_for_models(
    ...
    try:
        ...
        models_path = ApiDependencies.operator.services.configuration.models_path
        ...
        installed_models = ApiDependencies.operator.services.model_manager.store.search_by_attr()
...
async def update_model_record(
    ...
    logger = ApiDependencies.operator.services.logger
    record_store = ApiDependencies.operator.services.model_manager.store
    installer = ApiDependencies.operator.services.model_manager.install
    try:
...
async def get_model_image(
    ...
    try:
        path = ApiDependencies.operator.services.model_images.get_path(key)
...
async def update_model_image(
    ...
    logger = ApiDependencies.operator.services.logger
    model_images = ApiDependencies.operator.services.model_images
    try:
...
async def delete_model(
    ...
    deleted as well if they reside within the AI "models" directory.
    """
    logger = ApiDependencies.operator.services.logger
...
async def list_model_installs() -> List[ModelInstallJob]:
    ...
    * "cancelled" -- Job was cancelled before completion.
    ...
    """
    jobs: List[ModelInstallJob] = ApiDependencies.operator.services.model_manager.install.list_jobs()
    return jobs
...
async def get_model_install_job(id: int = Path(description="Model install id")) -> ModelInstallJob:
    ...
    """
    try:
        result: ModelInstallJob = ApiDependencies.operator.services.model_manager.install.get_job_by_id(id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
...
async def prune_model_install_jobs() -> Response:
    ...
    ApiDependencies.operator.services.model_manager.install.prune_jobs()
    return Response(status_code=204)
...
async def convert_model(
    ...
    """
    model_manager = ApiDependencies.operator.services.model_manager
    loader = model_manager.load
    logger = ApiDependencies.operator.services.logger
    store = ApiDependencies.operator.services.model_manager.store
    installer = ApiDependencies.operator.services.model_manager.install
    ...
    with TemporaryDirectory(dir=ApiDependencies.operator.services.configuration.models_path) as tmpdir:
    ...
    try:
        model_image = ApiDependencies.operator.services.model_images.get(key)
        ApiDependencies.operator.services.model_images.save(model_image, new_key)
        ApiDependencies.operator.services.model_images.delete(key)
...
async def get_cache_size(cache_type: CacheType = Query(description="The cache type", default=CacheType.RAM)) -> float:
    ...
    cache = ApiDependencies.operator.services.model_manager.load.ram_cache
    value = 0.0
...
@model_manager_router.put(
    ...
    response_model=float,
    summary="Set maximum size of model manager RAM or VRAM cache, optionally writing new value out to aittor.yaml config file.",
)
async def set_cache_size(
    ...
    persist: bool = Query(description="Write new value out to aittor.yaml", default=False),
) -> float:
...
async def get_stats() -> Optional[CacheStats]:
    ...
    return ApiDependencies.operator.services.model_manager.load.ram_cache.stats
