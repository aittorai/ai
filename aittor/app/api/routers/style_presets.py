...
from aittor.app.api.routers.model_manager import IMAGE_MAX_AGE
from aittor.app.services.style_preset_images.style_preset_images_common import StylePresetImageFileNotFoundException
...
async def get_style_preset(
    ...
    """Gets a style preset"""
    try:
        image = ApiDependencies.operator.services.style_preset_image_files.get_url(style_preset_id)
        style_preset = ApiDependencies.operator.services.style_preset_records.get(style_preset_id)
...
async def update_style_preset(
    ...
    if image is not None:
        ...
        try:
            ApiDependencies.operator.services.style_preset_image_files.save(style_preset_id, pil_image)
        except ValueError as e:
            raise HTTPException(status_code=409, detail=str(e))
    else:
        try:
            ApiDependencies.operator.services.style_preset_image_files.delete(style_preset_id)
    ...
    style_preset_image = ApiDependencies.operator.services.style_preset_image_files.get_url(style_preset_id)
    style_preset = ApiDependencies.operator.services.style_preset_records.update(
...
async def delete_style_preset(
    ...
    ApiDependencies.operator.services.style_preset_records.delete(style_preset_id)
...
async def create_style_preset(
    ...
    new_style_preset = ApiDependencies.operator.services.style_preset_records.create(style_preset=style_preset)
    ...
    if image is not None:
        ...
        try:
            ApiDependencies.operator.services.style_preset_image_files.save(new_style_preset.id, pil_image)
        except ValueError as e:
            raise HTTPException(status_code=409, detail=str(e))

    preset_image = ApiDependencies.operator.services.style_preset_image_files.get_url(new_style_preset.id)
...
async def list_style_presets() -> list[StylePresetRecordWithImage]:
    ...
    style_presets = ApiDependencies.operator.services.style_preset_records.get_many()
    for preset in style_presets:
        image = ApiDependencies.operator.services.style_preset_image_files.get_url(preset.id)
...
async def get_style_preset_image(
    ...
    try:
        path = ApiDependencies.operator.services.style_preset_image_files.get_path(style_preset_id)
...
async def export_style_presets():
    ...
    style_presets = ApiDependencies.operator.services.style_preset_records.get_many(type=PresetType.User)
...
async def import_style_presets(file: UploadFile = File(description="The file to import")):
    try:
        ...
        ApiDependencies.operator.services.style_preset_records.create_many(style_presets)
