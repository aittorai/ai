...
async def create_board(
    ...
    """Creates a board"""
    try:
        result = ApiDependencies.operator.services.boards.create(board_name=board_name)
        return result
    except Exception:
...
async def get_board(
    ...
    try:
        result = ApiDependencies.operator.services.boards.get_dto(board_id=board_id)
        return result
    except Exception:
...
async def update_board(
    ...
    """Updates a board"""
    try:
        result = ApiDependencies.operator.services.boards.update(board_id=board_id, changes=changes)
        return result
    except Exception:
...
async def delete_board(
    ...
    try:
        if include_images is True:
            deleted_images = ApiDependencies.operator.services.board_images.get_all_board_image_names_for_board(
                board_id=board_id
            )
            ApiDependencies.operator.services.images.delete_images_on_board(board_id=board_id)
            ApiDependencies.operator.services.boards.delete(board_id=board_id)
            ...
            deleted_board_images = ApiDependencies.operator.services.board_images.get_all_board_image_names_for_board(
                board_id=board_id
            )
            ApiDependencies.operator.services.boards.delete(board_id=board_id)
            return DeleteBoardResult(
...
async def list_boards(
    ...
    if all:
        return ApiDependencies.operator.services.boards.get_all(order_by, direction, include_archived)
    elif offset is not None and limit is not None:
        return ApiDependencies.operator.services.boards.get_many(order_by, direction, offset, limit, include_archived)
    else:
        raise HTTPException(
            status_code=400,
...
async def list_all_board_image_names(
    ...
    image_names = ApiDependencies.operator.services.board_images.get_all_board_image_names_for_board(
        board_id,
    )
    return image_names
