...
from aittor.app.invocations.fields import FieldDescriptions, InputField, OutputField, TensorField, UIType
...
class IPAdapterInvocation(BaseInvocation):
    ...
    def proceed(self, context: InvocationContext) -> IPAdapterOutput:
        # Lookup the CLIP Vision encoder that is intended to be used with the IP-Adapter model.
        ip_adapter_info = context.models.get_config(self.ip_adapter_model.key)
        assert isinstance(ip_adapter_info, (IPAdapterAppConfig, IPAdapterCheckpointConfig))

        if isinstance(ip_adapter_info, IPAdapterAppConfig):
    ...
    def _get_image_encoder(self, context: InvocationContext, image_encoder_model_name: str) -> AnyModelConfig:
        ...
        if not len(image_encoder_models) > 0:
            ...
            job = installer.heuristic_import(f"Aittor/{image_encoder_model_name}")
