...
def InputField(
    ...
    On calling of `aittor()`, however, those fields may be required.

    For example, consider an ResizeImageInvocation with an `image: ImageField` field.

    `image` is required during the call to `aittor()`, but when the python class is instantiated,
    ...
    for the `aittor()` function.

    If we use `typing.Optional` in the node class definition, the field will be typed as optional in the
    `aittor()` method, and we'll have to do a lot of runtime checks to ensure the field is present - or
    any static type analysis tools will complain.

    To get around this, in node class definitions, we type all fields correctly for the `aittor()` function,
    but secretly make them optional in `InputField()`. We also store the original required bool and/or default
    value. When we call `aittor()`, we use this stored information to do an additional check on the class.
    """
    ...
    if input is Input.Any or input is Input.Connection:
        ...
        if default is not PydanticUndefined:
            # Before app, we'll check for the original default value and set it on the field if the field has no value
