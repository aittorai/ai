...
CUSTOM_NODE_PACK_SUFFIX = "__operator-custom-node"
...
class UIConfigBase(BaseModel):
    ...
    node_pack: str = Field(description="The node pack that this node belongs to, will be 'aittor' for built-in nodes")
...
class BaseInvocation(ABC, BaseModel):
    ...
    def get_output_annotation(cls) -> BaseInvocationOutput:
        """Gets the invocation's output annotation (i.e. the return annotation of its `aittor()` method)."""
        return signature(cls.aittor).return_annotation
    ...
    @abstractmethod
    def proceed(self, context: InvocationContext) -> BaseInvocationOutput:
        """App with provided context and return outputs."""
        pass

    def aittor_internal(self, context: InvocationContext, services: "InvocationServices") -> BaseInvocationOutput:
        """
        Internal app method, calls `aittor()` after some prep.
        Handles optional fields that are required to call `aittor()` and invocation cache.
        """
...
def invocation(
    ...
    def wrapper(cls: Type[TBaseInvocation]) -> Type[TBaseInvocation]:
        ...
        # The node pack is the module name - will be "aittor" for built-in nodes
