...
from aittor.app.invocations.baseinvocation import BaseInvocation, BaseInvocationOutput
...
from aittor.app.services.invocation_stats.invocation_stats_common import GESStatsNotFoundError
from aittor.app.services.operator import Operator
from aittor.app.services.session_processor.session_processor_base import (
    InvocationServices,
    OnAfterRunNode,
    OnAfterRunSession,
    OnBeforeRunNode,
...
from aittor.app.services.session_processor.session_processor_common import CanceledException, SessionProcessorStatus
from aittor.app.services.session_queue.session_queue_common import SessionQueueItem, SessionQueueItemNotFoundError
from aittor.app.services.shared.graph import NodeInputError
from aittor.app.services.shared.invocation_context import InvocationContextData, build_invocation_context
from aittor.app.util.profiler import Profiler
...
class DefaultSessionRunner(SessionRunnerBase):
    ...
    def run_node(self, invocation: BaseInvocation, queue_item: SessionQueueItem):
        try:
            ...
            with self._services.performance_statistics.collect_stats(invocation, queue_item.session_id):
                ...
                # App the node
                output = invocation.aittor_internal(context=context, services=self._services)
    ...
    def _on_node_error(
        ...
        self._services.logger.error(
            f"Error while app session {queue_item.session_id}, invocation {invocation.id} ({invocation.get_type()}): {error_message}"
        )
...
class DefaultSessionProcessor(SessionProcessorBase):
    ...
    def start(self, operator: Operator) -> None:
        ...
        self._profiler = (
            Profiler(
                logger=self._operator.services.logger,
                output_dir=self._operator.services.configuration.profiles_path,
                prefix=self._operator.services.configuration.profile_prefix,
            )
            if self._operator.services.configuration.profile_graphs
            else None
        )

        self.session_runner.start(services=operator.services, cancel_event=self._cancel_event, profiler=self._profiler)
        self._thread = Thread(
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                try:
                    ...
                    self._queue_item = self._operator.services.session_queue.dequeue()

                    if self._queue_item is None:
                        # The queue was empty, wait for next polling interval or event to try again
                        self._operator.services.logger.debug("Waiting for next polling interval or event")
                        poll_now_event.wait(self._polling_interval)
                        continue

                    self._operator.services.logger.debug(f"Executing queue item {self._queue_item.item_id}")
                    cancel_event.clear()
            ...
            error_message = str(e)
            error_traceback = traceback.format_exc()
            self._operator.services.logger.error(f"Fatal Error in session processor {error_type}: {error_message}")
            self._operator.services.logger.error(error_traceback)
            pass
        finally:
            stop_event.clear()
            poll_now_event.clear()
    ...
    def _on_non_fatal_processor_error(
        ...
        self._operator.services.logger.error(f"Non-fatal error in session processor {error_type}: {error_message}")
        self._operator.services.logger.error(error_traceback)

        if queue_item is not None:
            # Update the queue item with the completed session & fail it
            queue_item = self._operator.services.session_queue.set_queue_item_session(
                queue_item.item_id, queue_item.session
            )
            queue_item = self._operator.services.session_queue.fail_queue_item(
