...
from aittor.app.services.image_files.image_files_common import ImageFileNotFoundException
from aittor.app.services.shared.sqlite_migrator.sqlite_migrator_common import Migration
...
class Migration2Callback:
    ...
    def _migrate_embedded_workflows(self, cursor: sqlite3.Cursor) -> None:
        """
        In the latest release, they changed how it handles embedded workflows. The `images` table in
        ...
        for idx, image_name in enumerate(pbar):
            ...
            try:
                ...
                continue
            if "app_workflow" in pil_image.info:
                try:
                    UnsafeWorkflowWithVersionValidator.validate_json(pil_image.info.get("app_workflow", ""))
