...
from aittor.app.services.download import DownloadQueueServiceBase, MultiFileDownloadJob
...
from aittor.app.services.model_records import DuplicateModelException, ModelRecordServiceBase
from aittor.app.services.model_records.model_records_base import ModelRecordChanges
...
from aittor.backend.model_manager.metadata.metadata_base import HuggingFaceMetadata
from aittor.backend.model_manager.probe import ModelProbe
from aittor.backend.model_manager.search import ModelSearch
from aittor.backend.util import AppLogger
from aittor.backend.util.catch_sigint import catch_sigint
from aittor.backend.util.devices import TorchDevice
from aittor.backend.util.util import slugify

TMPDIR_PREFIX = "tmpinstall_"


class ModelInstallService(ModelInstallServiceBase):
    """class for Aittor model installation."""
    ...
    def __init__(
        ...
        :param app_config: AIAppConfig object
        ...
        self._logger = AppLogger.get_logger(name=self.__class__.__name__)
    ...
    @property
    def app_config(self) -> AIAppConfig:  # noqa D102
    ...
    # make the operator optional here because we don't need it and it
    # makes the installer harder to use outside the web app
    def start(self, operator: Optional[Operator] = None) -> None:
    ...
    def stop(self, operator: Optional[Operator] = None) -> None:
    ...
    def delete(self, key: str) -> None:  # noqa D102
        ...
        if model_path.is_relative_to(self.app_config.models_path):
            # If the models is in the App-managed models dir, we delete it
    ...
    @classmethod
    def _download_cache_path(cls, source: Union[str, AnyHttpUrl], app_config: AIAppConfig) -> Path:
    ...
    def _register_orphaned_models(self) -> None:
        """Scan the App-managed models directory for orphaned models and registers them.
    ...
    def _register(
        ...
        # Models in the App-managed models dir should use relative paths.
        if model_path.is_relative_to(self.app_config.models_path):
        ...
        if isinstance(info, CheckpointConfigBase):
            ...
            # App-managed legacy config dir, we use a relative path.
