...
import aittor.configs as model_configs
from aittor.backend.model_hash.model_hash import HASHING_ALGORITHMS
from aittor.frontend.cli.arg_parser import AppConfigArgs

INIT_FILE = Path("aittor.yaml")
DB_FILE = Path("aittor.db")
LEGACY_INIT_FILE = Path("aittor.init")
...
class AIAppConfig(BaseSettings):
    """Ai global app configuration.

    Typically, you won't need to interact with this class directly. Instead, use the `get_config` function from `aittor.app.services.config` to get a singleton config object.
    ...
    Attributes:
        ...
        db_dir: Path to App databases directory.
    ...
    db_dir:                        Path = Field(default=Path("databases"),  description="Path to App databases directory.")
    ...
    model_config = SettingsConfigDict(env_prefix="AITTOR_", env_ignore_empty=True)

    def update_config(self, config: dict[str, Any] | AIAppConfig, clobber: bool = True) -> None:
        """Updates the config, overwriting existing values.

        Args:
            config: A dictionary of config settings, or instance of `AIAppConfig`. If an instance of \
                `AIAppConfig`, only the explicitly set fields will be merged into the singleton config.
    ...
    def write_file(self, dest_path: Path, as_example: bool = False) -> None:
        ...
        with open(dest_path, "w") as file:
            ...
            file.write("\n")
            file.write("# Put user settings here - see https://support.aittor.com/configuration/:\n")
    ...
    def config_file_path(self) -> Path:
        """Path to aittor.yaml, resolved to an absolute path.."""
    ...
    def db_path(self) -> Path:
        """Path to the aittor.db file, resolved to an absolute path.."""
    ...
    def find_root() -> Path:
        ...
        if os.environ.get("APP_ROOT"):
            ...
            root = Path("~/aittor").expanduser().resolve()
        return root


class DefaultAppConfig(AIAppConfig):
    """A version of `AIAppConfig` that does not automatically parse any settings from environment variables
    or any file.
...
def migrate_v3_config_dict(config_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    Returns:
        An `AIAppConfig` config dict.

    """
    parsed_config_dict: dict[str, Any] = {}
    for _category_name, category_dict in config_dict["Aittor"].items():
        for k, v in category_dict.items():
            ...
            elif k in AIAppConfig.model_fields:
                # skip unknown fields
...
def load_and_migrate_config(config_path: Path) -> AIAppConfig:
    ...
    Returns:
        An instance of `AIAppConfig` with the loaded and migrated settings.
    """
    ...
    migrated = False
    if "Aittor" in loaded_config_dict:
        migrated = True
    ...
    if migrated:
        ...
        try:
            ...
            migrated_config = DefaultAppConfig.model_validate(loaded_config_dict)
    ...
    try:
        ...
        config = AIAppConfig.model_validate(loaded_config_dict)
        assert (
...
def get_config() -> AIAppConfig:
    ...
    - Retrieves any provided CLI args from the AppConfigArgs class. It does not _parse_ the CLI args; that is done in the main entrypoint.
    ...
    if not AppConfigArgs.did_parse:
        return config
    ...
    example_config = DefaultAppConfig()
