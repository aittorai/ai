...
from aittor.app.util.thumbnails import get_thumbnail_name, make_thumbnail
...
class DiskImageFileStorage(ImageFileStorageBase):
    ...
    def save(
        ...
        try:
            ...
            if metadata is not None:
                info_dict["app_metadata"] = metadata
                pnginfo.add_text("app_metadata", metadata)
            if workflow is not None:
                info_dict["app_workflow"] = workflow
                pnginfo.add_text("app_workflow", workflow)
            if graph is not None:
                info_dict["app_graph"] = graph
                pnginfo.add_text("app_graph", graph)
            ...
            image.save(
                ...
                "PNG",
                pnginfo=pnginfo,
                compress_level=self.__operator.services.configuration.pil_compress_level,
            )
    ...
    def get_workflow(self, image_name: str) -> str | None:
        ...
        workflow = image.info.get("app_workflow", None)
    ...
    def get_graph(self, image_name: str) -> str | None:
        image = self.get(image_name)
        graph = image.info.get("app_graph", None)
