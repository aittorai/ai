...
from aittor.app.services.board_records.board_records_common import BoardRecordNotFoundException
from aittor.app.services.bulk_download.bulk_download_base import BulkDownloadBase
from aittor.app.services.bulk_download.bulk_download_common import (
    DEFAULT_BULK_DOWNLOAD_ID,
    BulkDownloadException,
    BulkDownloadParametersException,
    BulkDownloadTargetException,
)
from aittor.app.services.image_records.image_records_common import ImageRecordNotFoundException
...
class BulkDownloadService(BulkDownloadBase):
    ...
    def handler(
        ...
        try:
            ...
            self._signal_job_failed(bulk_download_id, bulk_download_item_id, bulk_download_item_name, e)
            self._operator.services.logger.error("Problem bulk downloading images.")
            raise e

    def _image_handler(self, image_names: list[str]) -> list[ImageDTO]:
        return [self._operator.services.images.get_dto(image_name) for image_name in image_names]

    def _board_handler(self, board_id: str) -> list[ImageDTO]:
        image_names = self._operator.services.board_image_records.get_all_board_image_names_for_board(board_id)
    ...
    def _get_clean_board_name(self, board_id: str) -> str:
        ...
        return self._clean_string_to_path_safe(self._operator.services.board_records.get(board_id).board_name)
    ...
    def _create_zip_file(self, image_dtos: list[ImageDTO], bulk_download_item_id: str) -> str:
        ...
        with ZipFile(zip_file_path, "w") as zip_file:
            for image_dto in image_dtos:
                ...
                image_disk_path = self._operator.services.images.get_path(image_dto.image_name)
    ...
    def _signal_job_started(
        ...
        if self._operator:
            ...
            self._operator.services.events.emit_bulk_download_started(
    ...
    def _signal_job_completed(
        ...
        if self._operator:
            ...
            self._operator.services.events.emit_bulk_download_complete(
                bulk_download_id, bulk_download_item_id, bulk_download_item_name
            )
    ...
    def _signal_job_failed(
        ...
        if self._operator:
            ...
            self._operator.services.events.emit_bulk_download_error(
