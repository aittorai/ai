...
from aittor.app.services.shared.graph import GraphExecutionState
...
class SqliteSessionQueue(SessionQueueBase):
    ...
    def start(self, operator: Operator) -> None:
        ...
        if self.__operator.services.configuration.clear_queue_on_startup:
            clear_result = self.clear(DEFAULT_QUEUE_ID)
            if clear_result.deleted > 0:
                self.__operator.services.logger.info(f"Cleared all {clear_result.deleted} queue items")
        else:
            prune_result = self.prune(DEFAULT_QUEUE_ID)
            if prune_result.deleted > 0:
                self.__operator.services.logger.info(f"Pruned {prune_result.deleted} finished queue items")
    ...
    def _set_in_progress_to_canceled(self) -> None:
        ...
        This is necessary because the operator may have been killed while processing a queue item.
        """
        try:
            self.__lock.acquire()
    ...
    def enqueue_batch(self, queue_id: str, batch: Batch, prepend: bool) -> EnqueueBatchResult:
        try:
            ...
            max_queue_size = self.__operator.services.configuration.max_queue_size
        ...
        self.__operator.services.events.emit_batch_enqueued(enqueue_result)
    ...
    def _set_queue_item_status(
        ...
        self.__operator.services.events.emit_queue_item_status_changed(queue_item, batch_status, queue_status)
        return queue_item
    ...
    def clear(self, queue_id: str) -> ClearResult:
        try:
            ...
            self.__lock.release()
        self.__operator.services.events.emit_queue_cleared(queue_id)
    ...
    def cancel_by_queue_id(self, queue_id: str) -> CancelByQueueIDResult:
        try:
            ...
            if current_queue_item is not None and current_queue_item.queue_id == queue_id:
                ...
                self.__operator.services.events.emit_queue_item_status_changed(
