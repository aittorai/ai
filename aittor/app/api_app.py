...
import aittor.backend.util.hotfixes  # noqa: F401 (monkeypatching on import)
import aittor.frontend.web as web_dir
from aittor.app.api.dependencies import ApiDependencies
from aittor.app.api.no_cache_staticfiles import NoCacheStaticFiles
from aittor.app.api.routers import (
    app_info,
    board_images,
    boards,
    download_queue,
...
from aittor.app.api.sockets import SocketIO
from aittor.app.services.config.config_default import get_config
from aittor.app.util.custom_openapi import get_openapi_func
...
if is_mps_available():
    import aittor.backend.util.mps_fixes  # noqa: F401 (monkeypatching on import)


logger = AppLogger.get_logger(config=app_config)
...
def overridden_swagger() -> HTMLResponse:
    return get_swagger_ui_html(
        ...
        swagger_favicon_url="static/docs/app-favicon-docs.svg",
    )
...
def overridden_redoc() -> HTMLResponse:
    return get_redoc_html(
        ...
        redoc_favicon_url="static/docs/app-favicon-docs.svg",
    )
...
def app_api() -> None:
    ...
    if app_config.dev_reload:
        try:
            ...
            jurigged.watch(logger=AppLogger.get_logger(name="jurigged").info)
    ...
    # replace uvicorn's loggers with app for consistent appearance
    for logname in ["uvicorn.access", "uvicorn"]:
        log = AppLogger.get_logger(logname)
        log.handlers.clear()
