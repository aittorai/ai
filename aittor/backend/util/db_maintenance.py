...
class ConfigMapper:
    ...
    def __load_from_root_config(self, app_root):
        ...
        if not os.path.exists(yaml_path):
            print(f"Unable to find aittor.yaml at {yaml_path}!")
            return False
        if os.path.exists(yaml_path):
            ...
            if db_dir is None:
                ...
                print(f"The aittor.yaml file was found but is missing the db_dir setting! Defaulting to {db_dir}")
            if outdir is None:
                outdir = self.DEFAULT_OUTDIR
                print(f"The aittor.yaml file was found but is missing the outdir setting! Defaulting to {outdir}")

            if os.path.isabs(db_dir):
                self.database_path = os.path.join(db_dir, self.DATABASE_FILENAME)
            else:
                self.database_path = os.path.join(app_root, db_dir, self.DATABASE_FILENAME)
            ...
            if os.path.isabs(outdir):
                ...
                self.outputs_path = os.path.join(app_root, outdir, "images")
                self.archive_path = os.path.join(app_root, outdir, "images-archive")
            ...
            if db_exists and outdir_exists:
                ...
                print(
                    "\nOne or more paths specified in aittor.yaml do not exist. Please inspect/correct the configuration and ensure the script is run in the developer console mode (option 8) from an App root directory."
                )
                return False
        else:
            print(
                f"Auto-discovery of configuration failed! Could not find ({yaml_path})!\n\nPlease ensure the script is run in the developer console mode (option 8) from an App root directory."
            )
            return False

    def __load_paths_from_yaml_file(self, yaml_path):
        """Load an App yaml file and get the database and outputs paths."""
        try:
...
class DatabaseMaintenanceApp:
    ...
    def main(self):  # noqa D107
        print("\n===============================================================================")
        print("Database and outputs Maintenance for App 3.0.0 +")
        print("===============================================================================\n")
...
def main():  # noqa D107
    parser = argparse.ArgumentParser(
        description="AI image database maintenance utility",
...
""",
    ...
    parser.add_argument("--root", default=".", type=Path, help="App root directory")
    parser.add_argument(
    ...
    try:
        os.chdir(args.root)
        app = DatabaseMaintenanceApp(args.operation)
        app.main()
