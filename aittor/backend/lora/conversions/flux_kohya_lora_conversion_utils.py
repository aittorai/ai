...
def lora_model_from_flux_kohya_state_dict(state_dict: Dict[str, torch.Tensor]) -> LoRAModelRaw:
    ...
    # Convert the state dict to the Aittor format.
    grouped_state_dict = convert_flux_kohya_state_dict_to_aittor_format(grouped_state_dict)
...
def convert_flux_kohya_state_dict_to_aittor_format(state_dict: Dict[str, T]) -> Dict[str, T]:
    """Converts a state dict from the Kohya FLUX LoRA format to LoRA weight format used internally by Aittor.
