...
const createSelector = (
  ...
  createMemoizedSelector(
    ...
    (system, nodes, workflowSettings, dynamicPrompts, canvas, params, upscale, config, activeTabName) => {
      ...
      if (!isConnected) {
        reasons.push({ content: i18n.t('parameters.app.systemDisconnected') });
      }

      if (activeTabName === 'workflows') {
        if (workflowSettings.shouldValidateGraph) {
          if (!nodes.nodes.length) {
            reasons.push({ content: i18n.t('parameters.app.noNodesInGraph') });
          }
          ...
          nodes.nodes.forEach((node) => {
            ...
            if (!nodeTemplate) {
              // Node type not found
              reasons.push({ content: i18n.t('parameters.aittor.missingNodeTemplate') });
              return;
            }
            ...
            forEach(node.data.inputs, (field) => {
              ...
              if (!fieldTemplate) {
                reasons.push({ content: i18n.t('parameters.aittor.missingFieldTemplate') });
                return;
              }

              if (fieldTemplate.required && field.value === undefined && !hasConnection) {
                reasons.push({
                  content: i18n.t('parameters.aittor.missingInputForField', {
        ...
        if (canvasIsFiltering) {
          reasons.push({ content: i18n.t('parameters.app.canvasIsFiltering') });
        }
        if (canvasIsTransforming) {
          reasons.push({ content: i18n.t('parameters.app.canvasIsTransforming') });
        }
        if (canvasIsRasterizing) {
          reasons.push({ content: i18n.t('parameters.app.canvasIsRasterizing') });
        }
        if (canvasIsCompositing) {
          reasons.push({ content: i18n.t('parameters.app.canvasIsCompositing') });
        }

        if (dynamicPrompts.prompts.length === 0 && getShouldProcessPrompt(positivePrompt)) {
          reasons.push({ content: i18n.t('parameters.aittor.noPrompts') });
        }

        if (!model) {
          reasons.push({ content: i18n.t('parameters.aittor.noModelSelected') });
        }

        if (model?.base === 'flux') {
          if (!params.t5EncoderModel) {
            reasons.push({ content: i18n.t('parameters.app.noT5EncoderModelSelected') });
          }
          if (!params.clipEmbedModel) {
            reasons.push({ content: i18n.t('parameters.app.noCLIPEmbedModelSelected') });
          }
          if (!params.fluxVAE) {
            reasons.push({ content: i18n.t('parameters.app.noFLUXVAEModelSelected') });
          }
          if (bbox.scaleMethod === 'none') {
            if (bbox.rect.width % 16 !== 0) {
              reasons.push({
                content: i18n.t('parameters.app.fluxModelIncompatibleBboxWidth', { width: bbox.rect.width }),
              });
            }
            if (bbox.rect.height % 16 !== 0) {
              reasons.push({
                content: i18n.t('parameters.app.fluxModelIncompatibleBboxHeight', { height: bbox.rect.height }),
              });
            }
          } else {
            if (bbox.scaledSize.width % 16 !== 0) {
              reasons.push({
                content: i18n.t('parameters.app.fluxModelIncompatibleScaledBboxWidth', {
            ...
            if (bbox.scaledSize.height % 16 !== 0) {
              reasons.push({
                content: i18n.t('parameters.app.fluxModelIncompatibleScaledBboxHeight', {
        ...
        canvas.controlLayers.entities
          ...
          .forEach((controlLayer, i) => {
            ...
            if (!controlLayer.controlAdapter.model) {
              problems.push(i18n.t('parameters.app.layer.controlAdapterNoModelSelected'));
            }
            // Model base must match
            if (controlLayer.controlAdapter.model?.base !== model?.base) {
              problems.push(i18n.t('parameters.app.layer.controlAdapterIncompatibleBaseModel'));
            }
            ...
            if (controlLayer.controlAdapter.type === 't2i_adapter') {
              ...
              if (bbox.scaleMethod === 'none') {
                if (bbox.rect.width % 16 !== 0) {
                  reasons.push({
                    content: i18n.t('parameters.app.layer.t2iAdapterIncompatibleBboxWidth', {
                      multiple,
                ...
                if (bbox.rect.height % 16 !== 0) {
                  reasons.push({
                    content: i18n.t('parameters.app.layer.t2iAdapterIncompatibleBboxHeight', {
                      multiple,
                ...
                if (bbox.scaledSize.width % 16 !== 0) {
                  reasons.push({
                    content: i18n.t('parameters.app.layer.t2iAdapterIncompatibleScaledBboxWidth', {
                      multiple,
                      width: bbox.scaledSize.width,
                    }),
                  });
                ...
                if (bbox.scaledSize.height % 16 !== 0) {
                  reasons.push({
                    content: i18n.t('parameters.app.layer.t2iAdapterIncompatibleScaledBboxHeight', {
                      multiple,
                      height: bbox.scaledSize.height,
                    }),
                  });
        ...
        canvas.referenceImages.entities
          ...
          .forEach((entity, i) => {
            ...
            if (!entity.ipAdapter.model) {
              problems.push(i18n.t('parameters.app.layer.ipAdapterNoModelSelected'));
            }
            // Model base must match
            if (entity.ipAdapter.model?.base !== model?.base) {
              problems.push(i18n.t('parameters.app.layer.ipAdapterIncompatibleBaseModel'));
            }
            // Must have an image
            if (!entity.ipAdapter.image) {
              problems.push(i18n.t('parameters.app.layer.ipAdapterNoImageSelected'));
            }
        ...
        canvas.regionalGuidance.entities
          ...
          .forEach((entity, i) => {
            ...
            if (entity.objects.length === 0) {
              problems.push(i18n.t('parameters.app.layer.rgNoRegion'));
            }
            ...
            if (
              ...
              problems.push(i18n.t('parameters.app.layer.rgNoPromptsOrIPAdapters'));
            }
