        ...
        <CommandItem key={item.value} value={item.value} onSelect={onSelect} asChild>
          <Flex role="button" flexDir="column" sx={cmdkItemSx} py={1} px={2} borderRadius="base">
            <Flex alignItems="center" gap={2}>
              {item.classification === 'beta' && <Icon boxSize={4} color="appYellow.300" as={PiHammerBold} />}
              {item.classification === 'prototype' && <Icon boxSize={4} color="appRed.300" as={PiFlaskBold} />}
              {item.classification === 'internal' && <Icon boxSize={4} color="appPurple.300" as={PiCircuitryBold} />}
