import { Icon, Tooltip } from '@aittorai/ui-library';
...
const ClassificationIcon = ({ classification }: { classification: Classification }) => {
  if (classification === 'beta') {
    return <Icon as={PiHammerBold} display="block" boxSize={4} color="appYellow.300" />;
  }

  if (classification === 'prototype') {
    return <Icon as={PiFlaskBold} display="block" boxSize={4} color="appRed.300" />;
  }

  if (classification === 'internal') {
    return <Icon as={PiCircuitryBold} display="block" boxSize={4} color="appPurple.300" />;
  }
