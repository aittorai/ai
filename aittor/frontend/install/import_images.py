...
class Config:
    ...
    AITTOR_DIRNAME = "aittor"
    YAML_FILENAME = "aittor.yaml"
    DATABASE_FILENAME = "aittor.db"
    ...
    def load_paths_from_yaml(self, yaml_path):
        """Load an AI yaml file and get the database and outputs paths."""
        try:
            with open(yaml_path, "rt", encoding=locale.getpreferredencoding()) as file:
                yamlinfo = yaml.safe_load(file)
                db_dir = yamlinfo.get("Aittor", {}).get("Paths", {}).get("db_dir", None)
                outdir = yamlinfo.get("Aittor", {}).get("Paths", {}).get("outdir", None)
...
class AppMetadata:
    """DTO for core generation properties parsed from metadata."""
...
class AppMetadataParser:
    """Parses strings with json data  to find AI core metadata properties."""
    ...
    def parse_meta_tag_aittor(self, tag_value):
        """Take as input an png metadata json node for the 'aittor' field variant from 3.0.0 beta 1 through 5"""
        props = AppMetadata()
...
VALUES ('{filename}', 'internal', 'general', {width}, {height}, null, null, '{metadata}', 0, '{modified_date_string}', '{modified_date_string}')"""
    ...
    def backup(self, timestamp_string):
        ...
        if not os.path.exists(self.database_backup_dir):
            ...
            print("Done!")
        database_backup_path = os.path.join(self.database_backup_dir, f"backup-{timestamp_string}-aittor.db")
...
class MediaImportProcessor:
        ...
        while True:
            ...
            while True:
                import_dir = os.path.expanduser(
                    prompt(
                        "Inputs: Specify absolute path containing .png images to import: ",
                        completer=PathCompleter(expanduser=True, only_directories=True),
                        default=default,
                    )
                )
    ...
    def import_image(self, filepath: str, board_name_option: str, db_mapper: DatabaseMapper, config: Config):
        ...
        if "app_metadata" in img_info:
            ...
            elif "aittor" in img_info:
                converted_field = parser.parse_meta_tag_aittor(json.loads(img_info.get("aittor")))
            elif "dream" in img_info:
                converted_field = parser.parse_meta_tag_dream(img_info.get("dream"))
            elif "Dream" in img_info:
                converted_field = parser.parse_meta_tag_dream(img_info.get("Dream"))
            else:
                converted_field = AppMetadata()
                destination_needs_meta_update = False
                print("File does not have metadata from known AI versions, add only, no update!")
        ...
        print(f"From AI Version {log_version_note} with dimensions {png_width} x {png_height}.")
        ...
        if destination_needs_meta_update:
            ...
            self.update_file_metadata_while_copying(
                filepath, file_destination_path, "app_metadata", latest_json_string
            )
            print("Done!")
        else:
    ...
    def update_file_metadata_while_copying(self, filepath, file_destination_path, tag_name, tag_value):
        ...
        with PIL.Image.open(filepath) as target_image:
            ...
            # re-add any existing app ai tags unless they are the one we are trying to add
            for key in existing_img_info:
                if key != tag_name and key in ("dream", "Dream", "sd-metadata", "aittor", "app_metadata"):
    ...
    def process(self):
        ...
        print("App into the currently installed root directory:")
        ...
        print("- AI metadata tags will be updated/written into the imported copy only.")
        print(
            "- On the imported copy, only known AI tags (latest and legacy) will be retained (dream, sd-metadata, aittor, app_metadata)"
        )
        print(
            "- A property 'imported_app_version' will be added to metadata that can be viewed in the UI's metadata viewer."
        )
        print(
            "- The new 3.x App outputs folder structure is flat so recursively found source imges will all be placed into the single outputs/images folder."
        )
